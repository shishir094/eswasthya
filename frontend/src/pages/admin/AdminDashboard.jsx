...
const AdminDashboard = () => {
  ...
  // Primary State ('users' | 'hospitals' | 'audit')
  ...
  const fetchAllData = useCallback(async () => {
    ...
    setError('');
    try {
      const [usersRes, hospitalsRes, auditRes] = await Promise.all([
        axios.get(`${API_BASE_URL}/admin/users`, { withCredentials: true }),
        axios.get(`${API_BASE_URL}/list`, { withCredentials: true }),
        axios.get(`${API_BASE_URL}/admin/audit-logs`, { withCredentials: true }).catch(() => ({ data: [] })),
      ]);
  ...
  // Fetch Audit Logs separately if needed or refreshed
  ...
  const activeDataset = activeTab === 'users' ? users : activeTab === 'hospitals' ? hospitals : [];

  const availableProvinces = useMemo(() => {
    if (activeTab === 'audit') return ['All'];
  ...
  }, [activeDataset, selectedProvince, activeTab]);
    ...
    return auditLogs.filter((log) => {
      if (searchQuery.trim() !== '') {
        ...
        return (
          ...
          (log.target_identifier || '').toLowerCase().includes(q) ||
    ...
    <div className="p-6 bg-slate-900 min-h-screen text-slate-100">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex border-b border-slate-800 space-x-4">
          ...
          <button onClick={() => handleTabChange('audit')} className={`pb-3 text-xs font-semibold cursor-pointer border-b-2 transition ${activeTab === 'audit' ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-400 hover:text-slate-200'}`}>
            Audit Logs ({auditLogs.length})
          </button>
        </div>

        {/* Filters (Hidden for Audit Logs Tab) */}
        {activeTab !== 'audit' && (
        ...
        {/* Audit Logs Tab View */}
        {activeTab === 'audit' && (
          <div className="bg-slate-800/60 border border-slate-700/60 rounded-2xl shadow-xl flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-700/80 bg-slate-900/50 flex justify-between items-center">
              <div>
                <h3 className="text-xs font-bold text-slate-100 uppercase tracking-wider">System Audit Trail</h3>
              ...
              <input 
                ...
                className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-blue-500 w-64" 
              />
            </div>
            ...
            <div className="p-0 overflow-auto">
              {auditLoading ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse text-xs">
                    <thead>
                      <tr className="border-b border-slate-700/80 bg-slate-950/60 text-slate-400 font-semibold uppercase tracking-wider">
                        <th className="p-4">Timestamp</th>
                        <th className="p-4">Officer</th>
                        <th className="p-4">Type</th>
                        <th className="p-4">Target</th>
                        <th className="p-4">Action</th>
                        <th className="p-4">Comments / Reason</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-700/50">
